...
class Attendance extends BaseController
{
	...
	public function verify(){
		...
		if(!empty($students) ) {
			...
			$schedule = $enrollModel->getStudentSchedule(date('l'),$students[0]['student_id']);
			...
			if(abs(intval($difference_minute)) <= 60){
				...
				$enrollModel->updateAccumulatedHours($new_required_hrs, $schedule['id']);
	...
	public function attendance_time_out(){
		...
		if(!empty($students) ) {
			...
			if ($attendance[0]['timeout'] == null && $attendance[0]['status'] !== 'absent') {
				...
				// 	return redirect()->to(base_url('attendance'));
				// }else{
					$_SESSION['error'] = "You cant time out, Please Time-in on another day! or You already Absent";
					$this->session->markAsFlashdata('error');
					return redirect()->to(base_url('attendance'));
				// }
			}
